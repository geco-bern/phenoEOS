# This script plot the maps showing geographical distributions and densities of the data. Outputs include ED Fig. 9.
...
map_pep_sites <- ggplot(data = world) + 
  ...
  geom_point(data = df_pep_agg, aes(x = lon, y = lat),size=.2,color="darkred",alpha=0.2) +
...
map_modis_sites <- ggplot(data = world) + 
  ...
  geom_point(data = df_modis_agg, aes(x = lon, y = lat),size=.2,color="darkred",alpha=0.3) +
  theme(legend.position="right", panel.background = element_rect(fill = "aliceblue"),
        axis.title=element_blank(),
        plot.title=element_text(size=7),plot.subtitle=element_text(size=6),
        axis.text=element_text(size=6))

figED9 <- map_pep_sites / map_modis_sites
figED9 <- figED9 + plot_layout(heights = c(1, 1),widths = c(1,0.7)) + 
  plot_annotation(tag_levels = 'A',tag_suffix = ')') & theme(plot.tag = element_text(size = 7))
figED9
ggsave("~/phenoEOS/manuscript/figures/ED_Fig9.jpg", width = 170, height = 160, units="mm",dpi=300)
ggsave("~/phenoEOS/manuscript/figures/ED_Fig9.eps", device=cairo_ps, width = 170, height = 160, units="mm", dpi=300)
